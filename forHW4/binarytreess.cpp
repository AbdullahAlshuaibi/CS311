...
if ((E == Root->Elem) && (Root->Left != NULL) || (Root->Right != NULL))   // ** if what you want to delete is the root
  {
if(Root->Left != NULL)// ** and it has only the left subtree
  {  //** change the root to the left child and return
Root = Root->Left; return;
}
if(Root->Right != NULL)// ** and it has only the right subtree
...
return; }
   ...
   if (E < V->Elem)
     {  cout << "...going to the left" << endl;
V = V->Left; V->Up = V; // ** update Parent and V here to go down
}
   else
     {  cout << "...going to the right" << endl;
V = V->Right; V->Up = V; // ** update Parent and V here to go down
}
...
{
if(V == NULL)// ** if V is a leaf (case 1)
  {
...
delete V; P->Right = NULL; // ** delete V and also make Parent's right NULL
 ...
 else // ** if V has just the left child so bypass V (case 2)
   {
...
while(V->Right != NULL)// ** while the right child of V is still available
  {
V = V->Right;  Parent = V; // ** update Parent and V to go to the right
}
