  ...
  if (V != NULL)
    {
      ...
      cout<< "Element: " << V->Elem <<endl; // ", Height: " << V->Height << ", Balance: " << V->Balance <<endl;  ** display V's element, height and balance and do endl;
...
void BST::DeleteVertex(elem_t E)
{
  ...
  else if (((E == Root->Elem)) && ((Root->Left != NULL) || (Root->Right != NULL)))   // ** if what you want to delete is the root
    {
  ...
  // Otherwise deleting something else
	  ...
	  if ( E == V->Elem)   // found it
	    ...
	    {  cout << "...going to the left" << endl;
	      V->Up = V; V = V->Left;  // ** update Parent and V here to go down
	    }
	  else
	    {  cout << "...going to the right" << endl;
	      V->Up = V; V = V->Right;  // ** update Parent and V here to go down
	    }
  ...
  if(V->Left == NULL && V->Right == NULL)// ** if V is a leaf (case 1)
    {
      ...
      if(V == P->Left)// ** if V is a left child of P
        {
	  delete V; P->Left = NULL; // ** delete V and also make Parent's left NULL
        }
  ...
  else if(V->Left != NULL && V->Right == NULL) // ** if V has just the left child so bypass V (case 2)
    {
      ...
      if(V == P->Right)// ** You need if then else to determine Parent's left or right
        {  
	  P->Right = V->Left; // ** should point to V's left child; Make the left child
	  V->Left = P;       // ** point UP to the parent
  ...
  else if(V->Left == NULL && V->Right != NULL)// ** if V has just the right child so bypass V (case 2)
    {
      ...
      if(V == P->Right) // ** You need if then else to determine Parent's left or right
        { 
	  P->Right = V->Right;// ** should point to V's right child; make the right child
	  V->Right = P;// ** point UP to the parent;
	  delete V;}// ** Be sure to delete V
      else {
	P->Left = V->Right;
	V->Right = P;
...
elem_t BST::findMax(Vertex *V)
{
  ...
  while(V->Right != NULL)// ** while the right child of V is still available
    {
      Parent = V; V = V->Right; // ** update Parent and V to go to the right
    }
